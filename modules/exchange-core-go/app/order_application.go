...
func (s *OrderAppService) PlaceOrder(ctx context.Context, input PlaceOrderInput) (PlaceOrderResult, error) {
	...
	if s.PrivateEventPublisher != nil {
		_ = s.PrivateEventPublisher.Publish(ctx, PrivateEvent{
			EventID:       uuid.New(),
			EventType:     events.TypeOrderAccepted,
			OccurredAt:    result.IncomingOrder.UpdatedAt,
			CorrelationID: result.IncomingOrder.ID,
			CausationID:   result.IncomingOrder.ID,
			Symbol:        string(result.IncomingOrder.Symbol),
			OrderID:       result.IncomingOrder.ID,
			UserID:        result.IncomingOrder.UserID,
			Version:       1,
...
func (s *OrderAppService) CancelOrder(ctx context.Context, orderID uuid.UUID) (order.Order, error) {
	...
	if s.PrivateEventPublisher != nil {
		_ = s.PrivateEventPublisher.Publish(ctx, PrivateEvent{
			EventID:       uuid.New(),
			EventType:     events.TypeOrderCanceled,
			OccurredAt:    canceled.UpdatedAt,
			CorrelationID: canceled.ID,
			CausationID:   canceled.ID,
			Symbol:        string(canceled.Symbol),
			OrderID:       canceled.ID,
