...
type Settings struct {
	...
	MatchingEngineGRPCAddress string
	LedgerServiceGRPCAddress  string
	PostgresDSN               string
	OrderStoreBackend         string
	LedgerStoreBackend        string
...
func Load(processName string) Settings {
	...
	return Settings{
		...
		OrderServiceGRPCAddress:   envOrDefault("ORDER_SERVICE_GRPC_ADDR", defaultGRPCAddress("order-service")),
		MatchingEngineGRPCAddress: envOrDefault("MATCHING_ENGINE_GRPC_ADDR", defaultGRPCAddress("matching-engine")),
		LedgerServiceGRPCAddress:  envOrDefault("LEDGER_SERVICE_GRPC_ADDR", defaultGRPCAddress("ledger-service")),
		PostgresDSN:               os.Getenv("POSTGRES_DSN"),
		OrderStoreBackend:         envOrDefault("ORDER_STORE_BACKEND", defaultOrderStoreBackend(processName)),
		LedgerStoreBackend:        envOrDefault("LEDGER_STORE_BACKEND", defaultLedgerStoreBackend(processName)),
		MigrationsDir:             envOrDefault("MIGRATIONS_DIR", "migrations"),
		NotificationGRPCAddress:   os.Getenv("NOTIFICATION_GRPC_ADDR"),
	}
}
...
func defaultLedgerStoreBackend(processName string) string {
	switch processName {
	case "ledger-service":
		return "memory"
	default:
